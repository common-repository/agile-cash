...
class AGC_Carousel {
	...
	public function get_next_address() {
		...
		while (!AGC_Cryptocurrencies::is_valid_wallet_address($this->cryptoId, $nextAddress)) {
	...
	private function increment_current_index() {		
		...
		elseif ($this->currentIndex == ($this->carouselSeats - 1)) {
			...
			AGC_Util::log(__FILE__, __LINE__, 'Invalid current index! Something went wrong, please contact plug-in support.');
		}
	}
}
