jQuery(document).ready(function($) {
	...
	var cryptoPaymentEnabledId = '#woocommerce_agc_gateway_enabled';
	...
	var hideRow = function (crypto, settingName) {
		var elementId = '#woocommerce_agc_gateway_' + crypto.id + '_' + settingName;
	...
	var isCryptoEnabled = function (crypto) {
		var cryptoEnabledId = '#woocommerce_agc_gateway_' + crypto.id + '_enabled';
		return $(cryptoEnabledId).is(':checked');
	}
	var isCryptoElectrumEnabled = function (crypto) {
		var cryptoElectrumEnabledId = '#woocommerce_agc_gateway_' + crypto.id + '_electrum_enabled';
		return $(cryptoElectrumEnabledId).is(':checked');
	}
	var isCryptoAutoPaymentEnabled = function (crypto) {
		var cryptoAutoPaymentEnabledId = '#woocommerce_agc_gateway_' + crypto.id + '_autopayment_enabled';
		return $(cryptoAutoPaymentEnabledId).is(':checked');
	}
	var isCryptoCarouselEnabled = function (crypto) {
		var cryptoMultiAddressEnabledId = '#woocommerce_agc_gateway_' + crypto.id + '_carousel_enabled';
		return $(cryptoMultiAddressEnabledId).is(':checked');
	}

	var setElectrumEnabled = function(crypto, isEnabled) {
		var elementId = '#woocommerce_agc_gateway_' + crypto.id + '_electrum_enabled';
		$(elementId).prop("checked", isEnabled);
	}

	var setAutoPaymentEnabled = function(crypto, isEnabled) {
		var elementId = '#woocommerce_agc_gateway_' + crypto.id + '_autopayment_enabled';
		$(elementId).prop("checked", isEnabled);
	}

	var setCarouselEnabled = function(crypto, isEnabled) {
		var elementId = '#woocommerce_agc_gateway_' + crypto.id + '_carousel_enabled';
		$(elementId).prop("checked", isEnabled);
	}
	...
	cryptos.forEach(function (crypto) {
		...
		var cryptoCarouselEnabledId = '#woocommerce_agc_gateway_' + crypto.id + '_carousel_enabled';
